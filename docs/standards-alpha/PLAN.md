...
**Что может быть в полёте.** На момент записи открыты два наших PR — #38
(корень `/` остаётся на driver, чистка устаревших вопросов) и #39 (запись
того, что жило только в переписке). Если их уже нет в списке открытых —
они смержены, это нормально. Если есть — их мерж и есть ближайший шаг.

Ещё пять PR от плагина (#1, #9, #10, #11, #14) висят открытыми с весны —
это Q-16, решение пользователя, а не работа.
