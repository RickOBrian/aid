...
## Волна 0.6 — переезд гейтов с Cursor на Claude
...
| ID | Задача | Статус |
|---|---|---|
| W06-1 | Решить судьбу `.cursor/rules/` — что переезжает, что остаётся легаси | blocked — вопрос Q-08 |
| W06-2 | Перенести содержание гейтов в `skills/_shared/protocols/gates/` инструмент-нейтрально | todo |
| W06-3 | Раздел «Гейты» в `CLAUDE.md`: триггер → файл; только `CLAUDE.md` гарантированно загружается | todo |
| W06-4 | Перенаправить ссылки из стандартов с `.cursor/rules/*.mdc` на новый дом | todo |
| W06-5 | Расширить `check-docs`: стандарт не должен ссылаться на легаси-пути | todo |
