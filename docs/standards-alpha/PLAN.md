...
**Следующая содержательная задача.** Из очереди ниже первой стоит брать
пространственные токены `driver`: пока вопрос не решён, любой новый
компонент формально расходится с `token-rules`. Это единственная запись в
очереди, которая мешает двигаться дальше; остальные можно брать в любом
порядке.
...
| Что | Откуда | Статус |
|---|---|---|
| Пространственные токены driver | Находка аудита №2 | Блокирует: новый компонент формально расходится с `token-rules` |
