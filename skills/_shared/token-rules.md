...
# Правила токенов дизайн-системы — v1.3.0
...
Типографика в компонентах работает по той же двухуровневой модели Core → Semantic.
Компоненты ссылаются только на semantic-типографику.

Semantic-токен типографики описывает полный стиль одной строкой:
`label-m` → font-family + font-size + line-height + font-weight + letter-spacing

Иерархия ролей:
- `display-*` — акцентные экраны (онбординг, промо). В компонентах не используется.
- `heading-*` — заголовки (1–2 строки): heading-xl, heading-l, heading-m, heading-s, heading-xs
- `body-*` — текст для чтения (2+ строки): body-l, body-m, body-s
- `label-*` — текст на контролах (кнопки, инпуты, чипы, меню): label-l, label-m, label-s, label-xs

Правила применения в компонентах:
- ButtonText, ButtonIcon, Chip, Tag, Tab, MenuItem → label-*
- Заголовок карточки, диалога, шторки → heading-*
- Описание, параграф, вспомогательный текст → body-*

### Типографические блокеры

| Нарушение | Пример | Как исправить |
|---|---|---|
| Захардкоженное значение | `font-size: 14px` | `label-m` |
| Компонент → Core напрямую | `button → type-size-14` | `button → label-m` |
| Body на контроле | `button → body-m` | `button → label-m` |
| Label в параграфе | `description → label-m` | `description → body-s` |
| Частичный токен | только size без LH/weight | задать полный semantic-токен |

Если нужного semantic-токена нет — отметить явно в спеке:
⚠️ нужно добавить токен: component-[entity]
и описать предлагаемый состав через Core-токены.
...
- **1.3.0** — добавлен раздел «Типографические токены»: блокеры, правила применения ролей в компонентах.
