...
## Шаг 0.1. Предусловие — компонент должен быть утверждён

Проверь `memory/ds-component-spec/log.*.json` на запись с `component ==
[ComponentName]`. Три случая:

- **Записи нет** → **останови работу**: «Спеки для `[ComponentName]` не
  найдено. Сначала запусти `ds-component-spec`, миграция без
  утверждённой спеки не выполняется.»
- **Запись есть, `stage: draft` (или есть `open_questions`)** → Discovery
  и Pilot разрешены, но только в **read-only-совместимом режиме**: перед
  выводом каждой фазы явно помечай
  «⚠️ spec draft, open questions: [список из спеки]». Pilot можно
  выполнять как черновой прогон, но **Batch Rollout (Phase 4)
  блокируется**, пока спека не переведена в `stage: final` — при попытке
  `/migrate_component rollout` останови с той же пометкой и открытыми
  вопросами.
- **Запись есть, `stage: final`** → работай без ограничений.
...
(вопросы заданы и отвечены, либо изначально не требовались).
...
Дополнительный гейт (см. Шаг 0.1): если спека `[ComponentName]` всё ещё
`stage: draft` — Rollout не выполняется, независимо от подтверждения.
...
**не трогай**, они остаются в follow-up (Phase 5).
